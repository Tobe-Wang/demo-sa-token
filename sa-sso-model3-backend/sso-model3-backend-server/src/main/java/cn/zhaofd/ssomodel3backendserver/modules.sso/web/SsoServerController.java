...
package cn.zhaofd.ssomodel3backendserver.modules.sso.web;
...
/**
 * Sa-Token-SSO Server端Controller
 */
@RestController
public class SsoServerController {
    ...
    private void configSso(SaSsoServerTemplate ssoServerTemplate) {
        ...
        // 配置：登录处理函数
        // 对应RestAPI 登录接口http://{host}:{port}/sso/doLogin
        ...
        ssoServerTemplate.messageHolder.addHandle("userinfo", (ssoTemplate, message) -> {
            ...
            // 自定义返回结果（模拟）
            return SaResult.ok().set("id", message.get("loginId")).set("name", "LinXiaoYu").set("sex", "女").set("age", 18);
        });
    }
}
